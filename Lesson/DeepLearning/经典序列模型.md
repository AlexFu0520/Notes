...
你之前学的 AR 模型，核心是**用固定窗口$τ$内的历史观测值$y_{t-1}​,y_{t-2}​,...,y_{t-\tau}$，通过线性组合直接预测当前值$y_{t}$**，但它有两个无法解决的硬伤，也是引入隐变量的核心原因：
1. **有限窗口的长依赖失效**：AR 模型的预测能力完全受限于窗口 $τ$ 。$τ$ 设小了，抓不住很久以前的关键信息（比如半年前的政策对当前销量的影响）；$τ$ 设大了，参数数量爆炸，OLS 估计失效，还会严重过拟合。
2. **无法捕捉隐式、非线性的复杂依赖**：AR 模型只能捕捉「$y_{t}$​ 和历史$y$ 值之间的显式线性关系」，但现实中时序数据的规律大多是**隐式、非线性的**—— 比如用户的品牌忠诚度、市场的周期规律、信号的隐藏趋势，这些信息没有直接的观测值，却藏在历史数据的变化里，根本无法用 “历史值乘系数求和” 的 AR 模型表达。
...
给你一个最通俗、最贴合你之前学习的定义：
$h_{t}$​是时序模型在t时刻的 **「记忆摘要」**—— 它把从序列开头到t时刻的所有历史信息，压缩成一个固定长度的向量，既包含了能直接观测到的历史y值，也提取出了历史数据里隐藏的、无法直接观测的规律（比如趋势、周期、隐性关联）。
...
#### 1. 隐藏状态更新公式：$h_{t}=g(h_{t-1}​,y_{t-1}​)$

这是模型的 **「记忆更新规则」**，也是整个隐变量模型的核心。

- 输入：两个核心信息
    - $h_{t-1}$​：t−1时刻的旧记忆，包含了到t−1时刻为止的所有历史信息；
    - $y_{t-1}$：t−1时刻刚观测到的新数据，是要补充进记忆里的新信息；
- 函数 $g$ ：**记忆更新函数**，它的作用是把「旧记忆」和「新观测值」融合，生成更新后的新记忆$h_{t}$。
    - 这个g可以是线性函数，也可以是非线性函数（比如神经网络）；
    - 你之前学的**RNN/LSTM 的核心，就是这个g的具体实现**：
        - 基础 RNN 的 $g$：$h_{t}​=tanh(W_{hh}h_{t}−1​+W_{y}h​_{yt}−1​+b_{h}​$)
        - LSTM 的 $g$ ：就是遗忘门、输入门、细胞状态更新、输出门的整套逻辑，本质还是用旧记忆和新观测值，更新新记忆。

#### 2. 预测公式：$y_{t}​=f(h_{t}​)+ϵ_{t}$​，其中$ϵ_{t}​∼N(0,σ^2)$且彼此独立

这是模型的 **「预测规则」**，用更新好的记忆，生成对当前值的预测。
- 函数 $f$ ：**预测函数**，输入是当前时刻的记忆ht​，输出是模型对yt​的预测值。
    - 比如 RNN 里的输出层：$f(h_{t})=W_{hy}h_{t}​+b_{y}​$，就是把隐藏状态映射成最终的预测结果；
    - 它可以是线性的，也可以是非线性的，完全适配复杂的预测需求。
- $ϵ_{t}$​：**随机噪声项**，是预测值和真实值之间的不可预测的随机误差，满足三个核心假设：
    1. 服从均值为 0、方差为σ2的正态分布；
    2. 不同时间步的 $ϵ_{t}$ ​彼此独立，没有自相关性；
    3. 它代表了模型无法捕捉的、纯随机的波动，所有能被历史信息解释的规律，都已经被压缩在 $h_{t}$ ​里了。
