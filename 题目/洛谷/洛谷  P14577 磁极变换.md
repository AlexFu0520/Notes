...
signed main(){
    ...
    for (re int i = 1;i <= n;i++){
        for (re int j = 0;j < 26;j++) pre[i][j] = pre[i - 1][j];
        pre[i][s[i] - 'a'] = i; pst[i] = pst[i - 1] ^ (1ll << (s[i] - 'a'));
    } q = read();
    for (re int i = 1,op,x,y;i <= q;i++){
        op = read(),x = read(),y = read();
        Q[i] = {op,{x,y}};
        if (op == 2) P[y].push_back({i,x});
    }
    for (re int i = 1;i <= n;i++){
        for (re int j = pre[i - 1][s[i] - 'a'] + 1;j <= i;j++) sst[j] |= (1 << (s[i] - 'a'));
        for (pii p:P[i]){
            for (re int j = 0;j < 26;j++){
                int x = pre[i][j];
                if (x < p.snd || !(((pst[x] ^ pst[p.snd - 1]) >> j) & 1)) continue;
                if (!((pst[x] ^ pst[p.snd - 1]) & sst[x + 1])) v[p.fst].push_back(x);
            }
        }
    }
    for (re int i = 1;i <= q;i++){
        if (Q[i].fst == 1) arr[Q[i].snd.fst] = Q[i].snd.snd;
        else{
            int ans = 0;
            for (int x:v[i]) ans += arr[x];
            printf("%lld\n",ans);
        }
    }
    return 0;
