...
## 思路：
1. 一道比较好的题，考虑二分答案。
2. 我们可以将边界设为 0 到 109 之间。
3. 由于一个老师当了班主任，会占用他原本可以用来教学一个班的能力。
4. 那么我们可以假设有 k 个班级，只要保证教学能力超过 k 即可。
5. 然后我们在处理班主任，看看可以使用的班主任是否大于等于 k 这个数量。
## ACcode:
```cpp
#include <iostream>
#include <vector>
...
const int N = 5e5 + 5;
ll master[N] = {0}; //每个学科班主任的数量
ll available[N] = {0}; //每个学科老师的数量
ll n,m,a,b,c;

bool check(int mid) {
    ll cnt = 0; //可用班主任
    for (int i = 1 ; i <= m ; ++i) {
        if (available[i] < mid) { //可用老师不够
            return false;
        }
        cnt += min(master[i],available[i] - mid); //累加班主任数量
    }
    return cnt >= mid; //可用班主任满足
}
